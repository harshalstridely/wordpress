...
if ( ! class_exists( 'strata_qode_sidebar' ) ) {
	class strata_qode_sidebar {
		var $sidebars = array();
		var $stored = "";
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'strata' );
		...
		function load_assets() {
			...
			wp_enqueue_script( 'strata-sidebar', QODE_ROOT . '/js/admin/qode-sidebar.js' );
			wp_enqueue_style( 'strata-sidebar', QODE_ROOT . '/css/admin/qode-sidebar.css' );
		}
		...
		function template_add_widget_field() {
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_attr__( 'Enter Name of the new Widget Area', 'strata' ) . "' name='qode-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_attr__( 'Add Widget Area', 'strata' ) . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'strata_qode_custom_widget_args', $args );
...
if ( ! function_exists( 'strata_qode_add_support_custom_sidebar' ) ) {
	function strata_qode_add_support_custom_sidebar() {
		add_theme_support( 'strata_qode_sidebar' );
		
		if ( get_theme_support( 'strata_qode_sidebar' ) ) {
			new strata_qode_sidebar();
		}
	}
	
	add_action( 'after_setup_theme', 'strata_qode_add_support_custom_sidebar' );
}