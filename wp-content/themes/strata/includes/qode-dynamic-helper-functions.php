...
if ( ! function_exists( 'strata_qode_is_css_folder_writable' ) ) {
	function strata_qode_is_css_folder_writable() {
		return is_writable( QODE_CSS_ROOT_DIR );
	}
}

if ( ! function_exists( 'strata_qode_is_js_folder_writable' ) ) {
	function strata_qode_is_js_folder_writable() {
		return is_writable( QODE_JS_ROOT_DIR );
	}
}

if ( ! function_exists( 'strata_qode_get_multisite_blog_id' ) ) {
	function strata_qode_get_multisite_blog_id() {
		if ( is_multisite() ) {
...
if ( ! function_exists( 'strata_qode_generate_dynamic_css_and_js' ) ) {
	/**
	 * Function that gets content of dynamic assets files and puts that in static ones
	 */
	function strata_qode_generate_dynamic_css_and_js() {
		global $wp_filesystem;
		WP_Filesystem();
		
		if ( strata_qode_is_css_folder_writable() ) {
			
			ob_start();
			include_once QODE_CSS_ROOT_DIR . '/style_dynamic.php';
			$css = ob_get_clean();
			if ( is_multisite() ) {
				$wp_filesystem->put_contents( QODE_CSS_ROOT_DIR . '/style_dynamic_ms_id_' . strata_qode_get_multisite_blog_id() . '.css', $css );
			} else {
			...
			$css = ob_get_clean();
			if ( is_multisite() ) {
				$wp_filesystem->put_contents( QODE_CSS_ROOT_DIR . '/style_dynamic_responsive_ms_id_' . strata_qode_get_multisite_blog_id() . '.css', $css );
			} else {
				$wp_filesystem->put_contents( QODE_CSS_ROOT_DIR . '/style_dynamic_responsive.css', $css );
			}
		}
		
		if ( strata_qode_is_js_folder_writable() ) {
			
			ob_start();
			include_once QODE_JS_ROOT_DIR . '/default_dynamic.php';
			$js = ob_get_clean();
			if ( is_multisite() ) {
				$wp_filesystem->put_contents( QODE_JS_ROOT_DIR . '/default_dynamic_ms_id_' . strata_qode_get_multisite_blog_id() . '.js', $js );
			} else {
	...
	add_action( 'strata_qode_action_after_theme_option_save', 'strata_qode_generate_dynamic_css_and_js' );
}

if ( ! function_exists( 'strata_qode_add_style_dynamic' ) ) {
	function strata_qode_add_style_dynamic() {
	...
	add_action( 'strata_qode_action_style_dynamic', 'strata_qode_add_style_dynamic' );
}

if ( ! function_exists( 'strata_qode_add_style_dynamic_responsive' ) ) {
	function strata_qode_add_style_dynamic_responsive() {
		$strata_options = strata_qode_return_global_options();
		?>
	...
	add_action( 'strata_qode_action_style_dynamic_responsive', 'strata_qode_add_style_dynamic_responsive' );
}

if ( ! function_exists( 'strata_qode_add_default_dynamic' ) ) {
	function strata_qode_add_default_dynamic() {
		$strata_options = strata_qode_return_global_options();
		?>
	...
	add_action( 'strata_qode_action_default_dynamic', 'strata_qode_add_default_dynamic' );
}