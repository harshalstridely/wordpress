...
if ( ! function_exists( 'strata_core_register_shortcodes_button' ) ) {
	function strata_core_register_shortcodes_button( $buttons ) {
...
if ( ! function_exists( 'strata_core_add_shortcodes_plugin' ) ) {
	function strata_core_add_shortcodes_plugin( $plugin_array ) {
		$plugin_array['qode_shortcodes'] = STRATA_CORE_MODULES_URL_PATH . '/shortcodes/qode_shortcodes.js';
		
		return $plugin_array;
	}
}

if ( ! function_exists( 'strata_core_shortcodes_button' ) ) {
	function strata_core_shortcodes_button() {
		if ( ! current_user_can( 'edit_posts' ) && ! current_user_can( 'edit_pages' ) ) {
			return;
		}
		
		if ( get_user_option( 'rich_editing' ) == 'true' ) {
			add_filter( 'mce_external_plugins', 'strata_core_add_shortcodes_plugin' );
			add_filter( 'mce_buttons', 'strata_core_register_shortcodes_button' );
		}
	}
	
	add_action( 'init', 'strata_core_shortcodes_button' );
}
...
if (!function_exists('image_with_text_over')) {
    ...
    function image_with_text_over($atts, $content = null) {
        ...
        if($icon_color != "") {
            $bcolor = strata_qode_hex2rgb($icon_color);
    ...
    function latest_post($atts, $content = null) {
	    ...
	    if($type != "boxes"){
		    $query_loop = new WP_Query(
			    array('post_status' => 'publish', 'orderby' => $order_by, 'order' => $order, 'posts_per_page' => $number_of_posts, 'category_name' => $category)
		    );
	    } else {
		    $query_loop = new WP_Query(
			    array('post_status' => 'publish', 'orderby' => $order_by, 'order' => $order, 'posts_per_page' => $number_of_colums, 'category_name' => $category)
		    );
	    }
	    ...
	    while ($query_loop->have_posts()) : $query_loop->the_post();
			...
			if($display_category == '1'){    
				$html .= '<span class="latest-vert-separator"> '.esc_html__("in", 'strata-core').'</span>';
				foreach ($cat as $categ) {
					$html .=' <a href="' . get_category_link($categ->term_id) . '">' . $categ->cat_name . ' </a> ';
				}
			}
            //generate comments part of description
            if (strata_qode_is_comments_enabled() && $display_category == "1") {
                $comments_count = get_comments_number();

                switch ($comments_count) {
                    case 0:
                        $comments_count_text = esc_html__('No comment', 'strata-core');
                        break;
                    case 1:
                        $comments_count_text = $comments_count . ' ' . esc_html__('Comment', 'strata-core');
                        break;
                    default:
                        $comments_count_text = $comments_count . ' ' . esc_html__('Comments', 'strata-core');
                        break;
                }
            ...
            if( strata_qode_is_like_enabled() && $display_like == '1'){
...
function line_graph($atts, $content = null) {
	...
	for ($i = 0 ; $i < count($line_graph_array) ; $i = $i + 1){
		...
		$color = strata_qode_hex2rgb(trim($line_graph_el[0]));
    ...
    function portfolio_list($atts, $content = null) {
        ...
        if ($filter == "yes") {
            ...
            $html .= "<div class='filter_holder'>
						<ul>
						<li class='label'><span data-label='". esc_html__('Filter','strata-core') ."'>". esc_html__('Filter','strata-core') ."</span><i class='fa fa-angle-down'></i>
						</li>
						<li class='filter' data-filter='all'><span>" . esc_html__('All', 'strata-core') . "</span></li>";
            if ($category == "") {
                $args = array(
                    'parent' => 0
                );
...
?>
            <p><?php esc_html_e('Sorry, no posts matched your criteria.', 'strata-core'); ?></p>
        <?php
        endif;
        ...
        if (get_next_posts_link(null, $query_loop->max_num_pages)) {
            if ($show_load_more == "yes" || $show_load_more == "") {
                if( is_front_page() || is_home() ){
                    global $paged;
                    $paged = $query_loop->query['paged'];
                }
                $html .= '<div class="portfolio_paging"><span rel="' . $query_loop->max_num_pages . '" class="load_more">' . get_next_posts_link(esc_html__('Show more', 'strata-core'), $query_loop->max_num_pages) . '</span></div>';
            }
        }
        $html .= "</div>";
	    wp_reset_postdata();
...
function portfolio_slider( $atts, $content = null ) {
    ...
    endwhile;
        ...
        $html .= esc_html__('Sorry, no posts matched your criteria.','strata-core');
    endif;
			...
			$html .= '<div class="social_share_holder">';
				$html .= '<a href="javascript:void(0)" target="_self"><span class="social_share_icon"></span>';
                    if($post_type == "post" || is_singular('portfolio_page')) {
                        $html .= '<span class="social_share_title">'.  esc_html__('Share','strata-core') .'</span>';
                    } 
                    $html .= '</a>';
					...
					if(isset($qode_options_theme13['enable_facebook_share']) &&  $qode_options_theme13['enable_facebook_share'] == "yes") { 
                        ...
                        if(wp_is_mobile()) {
                            $html .= '<a title="'.esc_html__("Share on Facebook",'strata-core').'" href="javascript:void(0)" onclick="window.open(\'https://m.facebook.com/sharer.php?u=' . urlencode(get_permalink());
                        }
                        else {
                            $html .= '<a title="'.esc_html__("Share on Facebook",'strata-core').'" href="javascript:void(0)" onclick="window.open(\'https://www.facebook.com/sharer.php?s=100&amp;p[title]=' . urlencode(esc_attr(get_the_title())) . '&amp;p[url]=' . urlencode(get_permalink()) . '&amp;p[images][0]=';
                        ...
                        $html .='\', \'sharer\', \'toolbar=0,status=0,width=620,height=280\');">';
        						...
        						$html .= "<span class='share_text'>" . esc_html__("Facebook",'strata-core') . "</span>";
    						$html .= "</a>";
						$html .= "</li>";
					} 
					...
					if($qode_options_theme13['enable_twitter_share'] == "yes") { 
                        ...
                        if(wp_is_mobile()) {
                            $html .= '<a href="#" onclick="popUp=window.open(\'https://twitter.com/intent/tweet?text=' . urlencode(strata_qode_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;">';
                        }
                        else {
                            $html .= '<a href="#" onclick="popUp=window.open(\'https://twitter.com/home?status=' . urlencode(strata_qode_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;">';
                        }
								...
								$html .= "<span class='share_text'>" . esc_html__("Twitter", 'strata-core') . "</span>";
							$html .= "</a>";
						$html .= "</li>";
					} 
                    if($qode_options_theme13['enable_google_plus'] == "yes") { 
                        $html .= '<li  class="google_share">';
                            $html .= '<a href="#" onclick="popUp=window.open(\'https://plus.google.com/share?url=' . urlencode(get_permalink()) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
                        		...
                        		$html .= "<span class='share_text'>" . esc_html__("Google+",'strata-core') . "</span>";
                        	$html .= "</a>";
                        $html .= "</li>";
                    }
					if(isset($qode_options_theme13['enable_linkedin']) && $qode_options_theme13['enable_linkedin'] == "yes") { 
						$html .= '<li  class="linkedin_share">';
                            $html .= '<a href="#" onclick="popUp=window.open(\'https://linkedin.com/shareArticle?mini=true&amp;url=' . urlencode(get_permalink()). '&amp;title=' . urlencode(get_the_title()) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
								...
								$html .= "<span class='share_text'>" . esc_html__("LinkedIn",'strata-core') . "</span>";
							$html .= "</a>";
						$html .= "</li>";
					}
					if(isset($qode_options_theme13['enable_tumblr']) && $qode_options_theme13['enable_tumblr'] == "yes") { 
						$html .= '<li  class="tumblr_share">';
					       	$html .= '<a href="#" onclick="popUp=window.open(\'https://www.tumblr.com/share/link?url=' . urlencode(get_permalink()). '&amp;name=' . urlencode(get_the_title()) .'&amp;description='.urlencode(get_the_excerpt()) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
								...
								$html .= "<span class='share_text'>" . esc_html__("Tumblr",'strata-core') . "</span>";
							$html .= "</a>";
						$html .= "</li>";
					}
					if(isset($qode_options_theme13['enable_pinterest']) && $qode_options_theme13['enable_pinterest'] == "yes") { 
						$html .= '<li  class="pinterest_share">';
                            ...
                            $html .= '<a href="#" onclick="popUp=window.open(\'https://pinterest.com/pin/create/button/?url=' . urlencode(get_permalink()). '&amp;description=' . esc_attr(get_the_title()) .'&amp;media='.urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
								...
								$html .= "<span class='share_text'>" . esc_html__("Pinterest",'strata-core') . "</span>";
							$html .= "</a>";
						$html .= "</li>";
					}
					if(isset($qode_options_theme13['enable_vk']) && $qode_options_theme13['enable_vk'] == "yes") { 
						$html .= '<li  class="vk_share">';
							...
							$html .= '<a href="#" onclick="popUp=window.open(\'https://vkontakte.ru/share.php?url=' . urlencode(get_permalink()). '&amp;title=' . urlencode(get_the_title()) .'&amp;description=' . urlencode(get_the_excerpt()) .'&amp;image='.urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
								...
								$html .= "<span class='share_text'>" . esc_html__("VK",'strata-core') . "</span>";
							$html .= "</a>";
						$html .= "</li>";
					}
					$html .= "</ul></div>";
...
function qode_slider( $atts, $content = null ) {
	...
	if ($slider != "") {
		...
		if((get_post_meta($page_id, "qode_header_color_transparency_per_page", true) == "" || get_post_meta($page_id, "qode_header_color_transparency_per_page", true) == "1") && ($qode_options_theme13['header_background_transparency_initial'] == "" || $qode_options_theme13['header_background_transparency_initial'] == "1")){
			...
			if (isset($qode_options_theme13['center_logo_image']) && $qode_options_theme13['center_logo_image'] == "yes" && isset($qode_options_theme13['logo_image'])) {
				$logo_width  = 0;
				$logo_height = 0;
				
				if ( ! empty( $qode_options_theme13['logo_image'] ) ) {
					$image_dimension = strata_qode_get_image_dimensions( $qode_options_theme13['logo_image'] );
        ...
        $title = get_the_title();
								...
								$html .= '</div><div class="video-wrap">
									...
									<video class="video" width="1920" height="800" poster="'.$video_image.'" controls="controls" preload="auto" loop autoplay muted>';
										 ...
										 $html .='<object width="320" height="240" type="application/x-shockwave-flash" data="'.get_template_directory_uri().'/js/flashmediaelement.swf">
													...
													<img src="'.$video_image.'" width="1920" height="800" title="' . esc_attr__( 'No video playback capabilities', 'strata-core' ) . '" alt="' . esc_attr__( 'Video thumb', 'strata-core' ) . '" />
											</object>
									</video>		
							</div></div>';
				}else{
			...
			endwhile;
				...
				$html .= esc_html__('Sorry, no slides matched your criteria.','strata-core');
			endif;
