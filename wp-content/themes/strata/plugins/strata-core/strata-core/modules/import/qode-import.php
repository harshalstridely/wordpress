...
class Qode_Import {
	...
	function init_qode_import() {
		if(isset($_REQUEST['import_option'])) {
			...
			if($import_option == 'content'){
				...
				$this->import_menus('menus.txt');
				$this->import_settings_pages('settingpages.txt');
				$this->message = esc_html__("Content imported successfully", 'strata-core');
			}
		}
	}
	...
	public function import_content( $file ) {
		...
		set_time_limit( 0 );
		$path = STRATA_CORE_MODULES_PATH . '/import/files/' . $file;
		
		$qode_import->fetch_attachments = $this->attachments;
		$returned_value                 = $qode_import->import( $path );
		if ( is_wp_error( $returned_value ) ) {
			$this->message = esc_html__( "An Error Occurred During Import", 'strata-core' );
		} else {
			$this->message = esc_html__( "Content imported successfully", 'strata-core' );
		}
		ob_get_clean();
	...
	public function import_widgets($file){
		...
		$this->message = esc_html__("Widgets imported successfully", 'strata-core');
	}
	...
	public function import_custom_sidebars($file){
		...
		$this->message = esc_html__("Custom sidebars imported successfully", 'strata-core');
	}
	
	public function import_options($file){
		$options = $this->file_options($file);
		update_option( 'qode_options_theme13', $options);
		$this->message = esc_html__("Options imported successfully", 'strata-core');
	}
	...
	public function file_options($file){
		$file_content = "";
		$file_for_import = STRATA_CORE_MODULES_PATH . '/import/files/' . $file;
		if ( file_exists($file_for_import) ) {
			$file_content = $this->qode_file_contents($file_for_import);
		} else {
			$this->message = esc_html__("File doesn't exist", 'strata-core');
		}
		if ($file_content) {
	...
	function qode_admin_import() {
		...
		$this->pagehook = add_submenu_page('qode_options_theme13_page', 'Qode Theme', esc_html__('Qode Import', 'strata-core'), 'manage_options', 'qode_options_import_page', array(&$this, 'qode_generate_import_page'));
	...
	function qode_generate_import_page() {
		...
		<div id="qode-metaboxes-general" class="wrap">
			<h2><?php esc_html_e('Strata - One-Click Import Demo Content', 'strata-core') ?></h2>
				...
				<div id="poststuff" class="metabox-holder">
					<div id="post-body" class="has-sidebar">
						<div id="post-body-content" class="has-sidebar-content">
							...
							<tbody>
								<tr valign="middle">
									<td scope="row" width="150"><?php esc_html_e('Import', 'strata-core'); ?></td>
									<td>
										<select name="import_option" id="import_option">
											<option value=""><?php esc_html_e( 'Please Select', 'strata-core' ); ?></option>
											<option value="complete_content"><?php esc_html_e( 'All', 'strata-core' ); ?></option>
											<option value="content"><?php esc_html_e( 'Content', 'strata-core' ); ?></option>
											<option value="widgets"><?php esc_html_e( 'Widgets', 'strata-core' ); ?></option>
											<option value="options"><?php esc_html_e( 'Options', 'strata-core' ); ?></option>
										</select>
										<input type="submit" value="Import" name="import" id="import_demo_data" />
									</td>
								</tr>
								<tr valign="middle">
									<td scope="row" width="150"><?php esc_html_e('Import attachments', 'strata-core'); ?></td>
									<td>
										<input type="checkbox" value="1" name="import_attachments" id="import_attachments" />
										
									</td>
								</tr>
								<tr class="loading-row"><td></td><td><div class="import_load"><span><?php esc_html_e('The import process may take some time. Please be patient.', 'strata-core') ?> </span><br />
								...
								<tr><td colspan="2">
									<?php esc_html_e('Important notes:', 'strata-core') ?><br />
									- <?php esc_html_e('Please note that import process will take time needed to download all attachments from demo web site.', 'strata-core'); ?><br />
									- <?php esc_html_e('If you plan to use shop, please install WooCommerce before you run import.', 'strata-core') ?>
								</td></tr>
